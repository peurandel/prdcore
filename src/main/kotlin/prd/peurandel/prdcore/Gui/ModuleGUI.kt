...
class ModuleGUI(plugin: JavaPlugin, database: MongoDatabase, suitUUID: String) : BaseGUI(plugin,"Module",54) {
    var database = database
    var suitUUID = suitUUID
    override fun initializeItems(plugin: JavaPlugin,player: String) {
        ...
        inventory.setItem(4, ModuleButton(plugin))
        ...
        inventory.setItem(48, Button().GoBack(plugin,"To Wardrobe Menu"))
    ...
    fun processButton(event: InventoryClickEvent,player : Player, buttonName : String) {
        when(buttonName) {
            "module" -> {player.sendMessage("HELLO!${getSuitUUID(event)}")}
            "engine" -> {
                ModuleEngineGUI(plugin,database,getSuitUUID(event)).open(plugin,player)
            }
            "armor" -> {
                ModuleArmorGUI(plugin,database,getSuitUUID(event)).open(plugin,player)
            }
            "skill" -> {
                ModuleTechGUI(plugin,database,getSuitUUID(event)).open(plugin,player)
            }
            "software" -> {
                ModuleSoftwareGUI(plugin,database,getSuitUUID(event)).open(plugin,player)
            }
            "magic" -> {
                ModuleMagicGUI(plugin,database,getSuitUUID(event)).open(plugin,player)
            }
            "orbital" -> {
                ModuleOrbitalGUI(plugin,database,getSuitUUID(event)).open(plugin,player)
            }
            "goback" -> {WardrobeGUI(plugin,database).open(plugin,player)}
    ...
    fun ModuleButton(plugin: JavaPlugin): ItemStack{
