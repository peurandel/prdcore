...
class ResearchTechGUI(plugin: JavaPlugin, private var database: MongoDatabase) : BaseGUI(plugin,"Research Tech",54) {
    ...
    override fun initializeItems(plugin: JavaPlugin, player: String) {
        ...
        inventory.setItem(4, Button().Skill(plugin))
        inventory.setItem(10, Button().skillPropellant(plugin))
        inventory.setItem(12, Button().skillFirearm(plugin))
    ...
    fun processButton(player : Player, buttonName : String) {
        when(buttonName) {
            ...
            "skill.propellant" -> {ResearchTechPropellantGUI(plugin,database).open(plugin,player)}
            "skill.firearm" -> {player.sendMessage("[${ChatColor.RED}!${ChatColor.WHITE}] 아직 개발중")}
        }
    }
